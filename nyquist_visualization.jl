...
md"""
# Nyquist Criterion: Visualizing the Argument Principle (General Cases)

This notebook demonstrates the **Argument Principle**, the mathematical basis of Nyquist analysis, using contours with different combinations of enclosed poles and zeros.
...
For a **clockwise** contour $C$ in the $s$-plane,

$$N = Z - P$$

where:
- $N$ = number of clockwise encirclements of the origin by $H(C)$,
- $Z$ = number of zeros of $H(s)$ inside $C$,
- $P$ = number of poles of $H(s)$ inside $C$.

Equivalently, the net phase change is

$$\Delta \angle H(s) = (P-Z)\times 360^\circ$$

for one clockwise traversal of $C$.
...
let
	...
	plot!(np, idx, Δp2_vals,
		lc=:purple, lw=2.5, ls=:dashdot, label="Net Δ∠(s − p₂)")
	plot!(np, idx, ΔH_vals,
		lc=:royalblue, lw=2.5, ls=:dash, label="Net Δ∠H(s)")
	hline!(np, [0.0],    lc=:black, lw=1, ls=:dot, label="")
	hline!(np, [-360.0], lc=:gray,  lw=1, ls=:dot, label="")
	...
	if t₂ == N_pts
		...
		ΔH  = round(ΔH_vals[end],  digits=0)
		annotate!(np, N_pts*0.48, Δp1_vals[end] + 30,
			text("Net Δ∠(s−p₁) = $(Δp1)°", 9, :crimson, :left))
		annotate!(np, N_pts*0.48, Δp2_vals[end] - 20,
...
let
	...
	if t₃ == N_pts
		...
		Δp1 = round(Δp1_vals[end], digits=0)
		ΔH  = round(ΔH_vals[end], digits=0)
		annotate!(np, N_pts*0.28, Δz_vals[end] + 25,
			text("Net Δ∠(s−z₁) = $(Δz)°", 9, :darkgreen, :left))
		annotate!(np, N_pts*0.28, Δp1_vals[end] - 25,
			text("Net Δ∠(s−p₁) = $(Δp1)°", 9, :crimson, :left))
		annotate!(np, N_pts*0.28, ΔH_vals[end] + 60,
			text("Net Δ∠H = $(ΔH)°  (cancellation)", 9, :royalblue, :left))
	end
